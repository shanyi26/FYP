...
The screenshots shown in the following sections are representative implementation views. Some were
captured while parts of the backend were still being finalized, so they show the implemented
frontend flow rather than a fully populated live contest environment.
...
This workflow is not fully linear. Participants may go back to the contest page, open another
problem, check a result, and return to the editor many times. Because of this, route structure,
shared layout, and page roles were treated as part of the system design, not only as UI choices.
...
The other option would have been faster page-by-page implementation with more local styling and
local logic. That might have been quicker at the start, but it would have made later alignment work
harder when several pages had to change in the same way.

== Authentication Pages

The authentication pages are the user's first entry point, so they were treated as part of the
overall frontend usability. The login page provides a simple sign-in form. The registration page is
for NTU users and validates username, NTU email format, password rules, and password confirmation
before submission.
...
The contest list page connects authentication with active participation. It loads contest data from
the API layer and shows each contest with its title, timing information, and current state.
Countdown labels and state indicators such as upcoming, ongoing, ended, or exited help users judge
when and how to enter a contest.

The contest join flow uses a password dialog because some contests should not be entered directly
from the list view. After the user joins a contest, the contest detail page becomes the main work
area. It combines the problem list and the clarification area on the same page.
...
This page also shows one of the main frontend-backend integration difficulties. The frontend has to
read not only contest metadata, but also participation state, such as whether the user has already
left the contest and which actions should still be shown.
...
#figure(
  ...
  caption: [Contest page combining problem access and clarifications],
)

The contest header shows the title, start time, end time, and contest-level actions such as the
exit flow.

== Problem Page

The problem page is the most important participant-facing page because it brings together reading,
coding, and submission preparation. The page reads both `contestId` and `problemId` from the route,
requests the problem from the backend, and renders the statement, constraints, examples, and code
editor.

This page was designed to keep reading and action close together. Instead of sending the user to a
separate submission screen right away, the editor stays next to the statement. The page supports
multiple programming languages, and the selected language and draft code are stored locally for
each problem.
...
The submit action is handled in the same page. After a successful request, the frontend goes
straight to the result view for the new submission.
...
In jury or root mode, the same page also shows editing controls for content maintenance. This means
the route has to handle role-sensitive behavior as well as normal participant behavior.

This page is also a clear example of frontend-backend coupling. The contest ID, problem ID, problem
payload, language handling, and submission endpoint all need to match. If one of them does not
match, the page may still look fine while the solve-and-submit flow breaks.

== Submission Views

Submission pages form the feedback loop of the participant workflow. The submissions page loads
contest-scoped submission data and shows it in a table with filters for problem, user, language,
and status.

The result page turns a submission record into an action point. Users can review the outcome, try
again, or go back to the submission list. This fits the way contest users usually work: submit,
check, revise, and submit again.

This loop also shows another implementation difficulty. The problem page creates a submission, the
result page reads the returned identifier and status, and the submissions view loads the same
attempt again as part of a list. These pages had to stay consistent with one another across route
changes.
...
The FAQ and profile pages are support pages rather than the center of the contest workflow, but
they help make the frontend feel like a complete application. The FAQ page provides guidance and a
way to ask questions. The profile page keeps account information in the same design style as the
contest pages.

Both pages also reuse the same routing, layout, and component patterns used in the rest of the
frontend.
